...
def test_local_sender_receiver_roundtrip():
    ...
    try:
        ...
        while time.time() - start_time < 5:
            ...
            if line:
                ...
                if "Listening on" in line:
                    started = True
                    break
        assert started, "Receiver did not start correctly. Output: " + "".join(collected)
        ...
        assert "[Sender] Sent successfully" in sender.stdout
        assert "Key:" in sender.stdout
        assert "IV:" in sender.stdout
        assert "Ciphertext:" in sender.stdout
        assert "[Receiver] Plaintext: Xin chao FIT4012 - local integration test" in full_receiver_output
    finally:
